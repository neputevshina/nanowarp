...
func NewWriter(w io.Writer, numSamples uint32, numChannels uint16, sampleRate uint32, bitsPerSample uint16, float bool) (writer *Writer) {
	blockAlign := numChannels * bitsPerSample / 8
	byteRate := sampleRate * uint32(blockAlign)
	var audioFormat uint16 = AudioFormatPCM
	if float {
		audioFormat = AudioFormatIEEEFloat
	}
	format := &WavFormat{audioFormat, numChannels, sampleRate, byteRate, blockAlign, bitsPerSample}
