...
// GrainSeeker allows to read a grain of size number of multichannel samples,
// from the input at the given offset from the start of the input.
//
...
func (m *monotonicGrainSeeker) GrainSeek(prr error, offset int64, size int) (buf [][]float64, err error) {
	...
	if size > len(m.tmp[0]) {
		for ch := range m.tmp {
			m.tmp[ch] = append(m.tmp[ch], make([]float64, size-len(m.tmp[0]))...)
		}
	}
	if size > len(m.lead[0]) {
		for ch := range m.lead {
			m.lead[ch] = append(m.lead[ch], make([]float64, size-len(m.lead[0]))...)
		}
	}
	...
	if m.first {
		m.first = false
		refill = true
	} else if len(m.tmp[0][intmp:]) < size {
		for ch := range nch {
			copy(m.tmp[ch], m.tmp[ch][intmp:]) // =: n
		}
		m.start = offset
		m.level = int64(len(m.tmp[0]) - intmp) // == n
		refill = true
	}
	if refill {
		if m.ended == 0 {
