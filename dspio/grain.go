...
func (m *monotonicGrainSeeker) GrainSeek(prr error, offset int64, buf [][]float64) error {
	// Generated by free tier of ChatGPT.
	if prr != nil {
		return prr
	}
	if len(buf) != m.nch {
		panic(fmt.Errorf(`different number of channels: expected %d, got %d`, m.nch, len(buf)))
	}
	...
	// Лениво создаём временный буфер.
	if m.tmp == nil || len(m.tmp) != len(buf) || len(m.tmp[0]) != nframe {
		m.tmp = make([][]float64, len(buf))
		for ch := range m.tmp {
			m.tmp[ch] = make([]float64, nframe)
		}
	}

	// Проматываем поток.
	for m.pos < offset {
		want := int(offset - m.pos)
		if want > nframe {
			want = nframe
		}
		...
		n, err := m.r.SignalRead(prr, tmp)
		m.pos += int64(n)

		if err != nil {
			if err == io.EOF && m.pos == offset {
				break
			}
			return err
		}
